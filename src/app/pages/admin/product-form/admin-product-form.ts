...
export class AdminProductForm implements OnInit {
  ...
  readonly productForm = this.formBuilder.group({
    ...
    volume: this.formBuilder.nonNullable.control('', Validators.maxLength(100)),
    blankSize: this.formBuilder.nonNullable.control('', Validators.maxLength(100)),
    includedBlankCount: this.formBuilder.nonNullable.control(1, [Validators.min(1), Validators.max(3)]),
  ...
  submit(): void {
    ...
    const request: ProductUpsertRequest = {
      ...
      blankSize: value.type === 'BLANK' ? value.blankSize.trim() : '',
      includedBlankCount: value.type === 'KIT' ? value.includedBlankCount : null,
  ...
  private fillForm(product: ProductDetail): void {
    this.productForm.patchValue({
      ...
      blankSize: product.blankSize ?? product.volume ?? '',
      includedBlankCount: product.includedBlankCount ?? 1,
